...
void print_secret(char* type, unsigned char* secret, int len){
	int i=0;
	printf("%s ", type);
	for(i=0;i<len;i++){
...
int ssh_generate_session_keys(ssh_session session)
{
    ...
    if (session->client) {
    	...
    	print_secret("Enckey server to client:", enckey_srv_to_cli, enckey_srv_to_cli_len);
    	print_secret("Enckey client to server:", enckey_cli_to_srv, enckey_cli_to_srv_len);
	print_secret("IV server to client:", IV_srv_to_cli, IV_len);
	print_secret("IV client to server:", IV_cli_to_srv, IV_len);
	rc = 0;
error:
